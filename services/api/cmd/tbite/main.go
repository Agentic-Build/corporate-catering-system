...
func runAPI(ctx context.Context, logger *slog.Logger, cfg config.Config) error {
	...
	if err != nil {
		return fmt.Errorf("pg ro pool: %w", err)
	}
	defer roPool.Close()
	...
	if cfg.HydraPublicURL != "" {
		...
		if err != nil {
			return fmt.Errorf("hydra access token verifier: %w", err)
		}
		...
		if len(cfg.AuthProviders) > 0 {
			...
			if err != nil {
				return fmt.Errorf("mcp oidc provider: %w", err)
			}
			bridgeOIDC = bp
		...
		if err != nil {
			return fmt.Errorf("hydra reverse proxy: %w", err)
		}
		hydraProxy = proxy
