...
func migrationsDir() string {
	...
	// services/api/internal/feedback/postgres/testhelper_test.go → ../../../../../migrations
...
VALUES ($1, $2, 'employee')
RETURNING id`,
		fmt.Sprintf("feedback-pg-user-%d@test.com", n),
		fmt.Sprintf("feedback-pg-user-%d", n),
	).Scan(&id)
...
RETURNING id`,
		fmt.Sprintf("feedback-pg-vendor-%d", n),
		fmt.Sprintf("feedback-pg-vendor-%d Ltd", n),
		fmt.Sprintf("feedback-pg-vendor-%d@test.com", n),
	).Scan(&id)
	require.NoError(t, err)
	return id
}

// seedOrder inserts a picked_up order row that ratings/complaints can
// reference, and returns its UUID.
...
VALUES ($1,$2,'F12B-3F',$3,'picked_up',$4,now(),$5,$6)
RETURNING id`,
		userID, vendorID, day, int64(12000), day.Add(10*time.Hour), secret,
	).Scan(&id)
	require.NoError(t, err)
	return id
}
