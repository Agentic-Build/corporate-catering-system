...
// handlePastExpiry flips a past-expiry document's status, opens the critical
// anomaly, and records the metric. Returns true when a row was handled.
func (s *DocumentExpiryScanner) handlePastExpiry(ctx context.Context, d *compliance.Document) bool {
...
// severityForDaysUntil buckets the doc-expiring anomaly severity.
func severityForDaysUntil(daysUntil int) compliance.AnomalySeverity {
	switch {
	case daysUntil <= 1:
		return compliance.SeverityCritical
	case daysUntil <= 7:
		return compliance.SeverityHigh
	default:
		return compliance.SeverityMedium
	}
}

// handleExpiringSoon opens an expiring-soon anomaly for d. Returns true when a
// row was emitted (skips past-expiry rows already handled in the first pass).
func (s *DocumentExpiryScanner) handleExpiringSoon(ctx context.Context, d *compliance.Document, now time.Time) bool {
...
func (s *DocumentExpiryScanner) RunOnce(ctx context.Context) (int, error) {
	...
	for _, d := range pastExpiry {
		if s.handlePastExpiry(ctx, d) {
			handled++
		}
	}
	for _, d := range expiringSoon {
		if s.handleExpiringSoon(ctx, d, now) {
			handled++
		}
	}
