...
{:else}
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
    {#each data.items as item (item.id)}
      <div class="flex flex-col overflow-hidden rounded-tb-2xl border border-tb-slate-200 bg-white shadow-tb-sm">
        <!-- 主圖 -->
        {#if item.images?.[0]}
        ...
        <div class="flex items-center justify-end gap-2 border-t border-tb-slate-100 px-4 py-2">
          <form method="POST" action="?/copy">
            ...
            <button type="submit" class="text-sm font-semibold text-tb-slate-500 hover:text-tb-slate-800">
              複製
            </button>
          </form>
          <a href="/menus/{item.id}" class="text-sm font-semibold text-tb-red-600 hover:text-tb-red-700">
            編輯
          </a>
          <form method="POST" action="?/delete">
            <input type="hidden" name="id" value={item.id} />
            <button type="submit" class="text-sm font-semibold text-tb-slate-400 hover:text-red-600">
              刪除
            </button>
          </form>
        </div>
