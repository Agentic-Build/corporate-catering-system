...
export const actions: Actions = {
  ...
  verifyPickup: async ({ request, locals }) => {
    const fd = await request.formData();
    const id = String(fd.get("order_id") ?? "");
    const code = String(fd.get("code") ?? "").trim();
    if (!id || !code) return fail(400, { error: "order_id + code required" });
    const client = apiFor(locals.apiToken);
    const r = await client.POST("/api/merchant/orders/{id}/verify-pickup", {
      params: { path: { id } } as any,
      body: { code } as any,
    });
    if (r.error) return fail(400, { error: "核銷失敗：請確認 6 位數動態碼" });
    return { success: true, verifiedID: id };
  },
};
