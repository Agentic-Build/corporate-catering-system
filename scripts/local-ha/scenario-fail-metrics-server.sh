...
metrics_namespace="${METRICS_NAMESPACE:-kube-system}"
metrics_deployment="${METRICS_DEPLOYMENT:-metrics-server}"
...
cpu_hpa_regex="^${release}-tbite-platform-(api|realtime|web-admin|web-employee|web-merchant)$"
...
dashboard_cpu_metrics_path_unavailable_seconds_10m() {
  dashboard_target_value "Autoscaler activity" "cpu metrics path unavailable seconds / 10m"
}
...
dashboard_cpu_hpa_inactive_seconds_10m() {
  promql_value "(sum_over_time(((sum(kube_horizontalpodautoscaler_status_condition{namespace=\"${namespace}\",horizontalpodautoscaler=~\"${release}-tbite-platform-(api|realtime|web-.*)\",condition=\"ScalingActive\",status=\"false\"} == 1) or vector(0)) > bool 0)[10m:15s]) or vector(0)) * 15"
}
...
dashboard_metrics_server_missing() {
  promql_value "((max(kube_deployment_status_replicas_available{namespace=\"${metrics_namespace}\",deployment=\"${metrics_deployment}\"}) or vector(0)) < bool 1)"
}

dashboard_metrics_server_unavailable_seconds_10m() {
  promql_value "(sum_over_time(((max(kube_deployment_status_replicas_available{namespace=\"${metrics_namespace}\",deployment=\"${metrics_deployment}\"}) or vector(0)) < bool 1)[10m:15s]) or vector(0)) * 15"
}
...
dashboard_keda_metrics_apiserver_missing() {
  promql_value "((max(kube_deployment_status_replicas_available{namespace=\"${namespace}\",deployment=\"keda-operator-metrics-apiserver\"}) or vector(0)) < bool 1)"
}
...
print_dashboard_signals() {
  ...
  printf 'dashboard_cpu_metrics_path_unavailable_seconds_10m=%s\n' "$(dashboard_cpu_metrics_path_unavailable_seconds_10m)"
  ...
  printf 'dashboard_cpu_hpa_inactive_seconds_10m=%s\n' "$(dashboard_cpu_hpa_inactive_seconds_10m)"
  printf 'dashboard_keda_hpas_inactive=%s\n' "$(dashboard_keda_hpas_inactive)"
  printf 'dashboard_metrics_server_missing=%s\n' "$(dashboard_metrics_server_missing)"
  printf 'dashboard_metrics_server_unavailable_seconds_10m=%s\n' "$(dashboard_metrics_server_unavailable_seconds_10m)"
...
cpu_hpa_count() {
  kubectl -n "${namespace}" get hpa -o json \
    | jq --arg re "${cpu_hpa_regex}" '[.items[] | select(.metadata.name | test($re))] | length'
}

cpu_hpa_inactive_count() {
  kubectl -n "${namespace}" get hpa -o json \
    | jq --arg re "${cpu_hpa_regex}" '
        [
          .items[]
          | select(.metadata.name | test($re))
          | .status.conditions[]?
          | select(.type == "ScalingActive" and .status != "True" and (.reason // "") == "FailedGetResourceMetric")
        ]
        | length
      '
}

print_hpa_conditions() {
  kubectl -n "${namespace}" get hpa -o json \
    | jq -r --arg re "${cpu_hpa_regex}" '
        .items[]
        | select(.metadata.name | test($re))
        | [
            .metadata.name,
...
wait_for_cpu_hpa_inactive() {
  local expected="$1"
  local deadline=$((SECONDS + timeout_seconds))
  local inactive
  while (( SECONDS < deadline )); do
    inactive="$(cpu_hpa_inactive_count)"
    if (( inactive >= expected )); then
      return 0
    fi
    sleep 5
  done
  echo "timed out waiting for CPU HPAs to report FailedGetResourceMetric" >&2
  kubectl -n "${namespace}" get hpa -o wide >&2 || true
  print_hpa_conditions >&2 || true
  return 1
}

wait_for_cpu_hpa_active() {
  local deadline=$((SECONDS + timeout_seconds))
  local inactive
  while (( SECONDS < deadline )); do
    inactive="$(cpu_hpa_inactive_count)"
  ...
  echo "timed out waiting for CPU HPAs to recover valid resource metrics" >&2
  kubectl -n "${namespace}" get hpa -o wide >&2 || true
  print_hpa_conditions >&2 || true
  return 1
}
...
original_replicas="$(
  kubectl -n "${metrics_namespace}" get deployment "${metrics_deployment}" -o jsonpath='{.spec.replicas}'
)"
...
cleanup() {
  ...
  if [[ "${restore}" != "true" ]]; then
    echo "RESTORE=${restore}; leaving ${metrics_namespace}/${metrics_deployment} scaled down"
  else
    echo "==> restoring ${metrics_namespace}/${metrics_deployment} to replicas=${original_replicas}"
    kubectl -n "${metrics_namespace}" scale deployment "${metrics_deployment}" --replicas="${original_replicas}" >/dev/null || status=1
    kubectl -n "${metrics_namespace}" rollout status "deployment/${metrics_deployment}" --timeout="${timeout_seconds}s" || status=1
    wait_for_cpu_hpa_active || status=1
    if [[ -n "${vm_url}" ]]; then
...
expected_hpas="$(cpu_hpa_count)"
if (( expected_hpas == 0 )); then
  echo "no CPU-backed platform HPAs found in namespace ${namespace}" >&2
  exit 1
fi

baseline_inactive="$(cpu_hpa_inactive_count)"
if (( baseline_inactive != 0 )); then
  echo "CPU HPAs are already inactive before the drill; refusing to mask a pre-existing autoscaler failure" >&2
  kubectl -n "${namespace}" get hpa -o wide >&2 || true
  print_hpa_conditions >&2 || true
  exit 1
fi

echo "==> baseline CPU HPA conditions"
start_vm_port_forward
...
echo "==> scaling ${metrics_namespace}/${metrics_deployment} to zero"
kubectl -n "${metrics_namespace}" scale deployment "${metrics_deployment}" --replicas=0

echo "==> waiting for ${expected_hpas} CPU HPAs to report FailedGetResourceMetric"
wait_for_cpu_hpa_inactive "${expected_hpas}"
kubectl -n "${namespace}" get hpa -o wide
print_hpa_conditions

echo "==> waiting for dashboard autoscaler signals to reflect the metrics-server outage"
wait_for_dashboard_at_least "dashboard_metrics_server_missing" dashboard_metrics_server_missing 1
wait_for_dashboard_at_least "dashboard_cpu_hpas_inactive" dashboard_cpu_hpas_inactive "${expected_hpas}"
...
wait_for_dashboard_at_least "dashboard_cpu_metrics_path_unavailable" dashboard_cpu_metrics_path_unavailable 1
wait_for_dashboard_at_most "dashboard_keda_metrics_path_unavailable" dashboard_keda_metrics_path_unavailable 0
wait_for_dashboard_at_least "dashboard_metrics_server_unavailable_seconds_10m" dashboard_metrics_server_unavailable_seconds_10m "${min_range_signal_seconds}"
wait_for_dashboard_at_least "dashboard_cpu_hpa_inactive_seconds_10m" dashboard_cpu_hpa_inactive_seconds_10m "${min_range_signal_seconds}"
wait_for_dashboard_at_least "dashboard_cpu_metrics_path_unavailable_seconds_10m" dashboard_cpu_metrics_path_unavailable_seconds_10m "${min_range_signal_seconds}"
...
echo "==> holding metrics-server outage for ${hold_seconds}s after dashboard detection"
sleep "${hold_seconds}"

echo "==> metrics-server outage observed"
