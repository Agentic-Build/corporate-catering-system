...
<span
  class="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-semibold {tones[
    tone
  ]}"
>
  <span
    class="h-1.5 w-1.5 rounded-full {dots[tone]} {pulse ? 'animate-pulse' : ''}"
    aria-hidden="true"
  ></span>
  {@render children?.()}
</span>
